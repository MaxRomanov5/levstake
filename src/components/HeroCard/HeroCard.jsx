...
<Stack sx={{marginBottom:'32px'}}>
    <Typography variant='h1' color='primary' sx={{marginBottom:'7px',fontSize:'64px'}}>{pool?.profit_rate ? Number(pool?.profit_rate).toFixed(0)+'%' :'-'}</Typography>
...
<Typography  variant='h2' sx={{fontSize:"24px",lineHeight:'31px',color:'primary.main'}}>{pool?.pool_conditions ? `${pool?.pool_conditions?.min_leverage}x-${pool?.pool_conditions?.max_leverage}x`:'-'}</Typography>
</Stack>
<Stack flexDirection='row' sx={{marginBottom:'8px'}} justifyContent='space-between'>
